...
def fig_annual_pl_chart():
    ...
    ebitda = [1820,  6480,  14400, 24000,  40000]
...
def slide_group_annual_pl(prs):
    ...
    rows = [
        ...
        ["　粗利率",              "69%",     "69%",     "69%",     "69%",     "69%"],
        ["　人件費",              "▲3,840",  "▲9,200",  "▲16,480", "▲29,000", "▲46,000"],
        ["　家賃合計",            "▲1,500",  "▲3,600",  "▲7,200",  "▲11,000", "▲17,000"],
        ["　その他（光熱費等）",  "▲2,500",  "▲5,560",  "▲11,600", "▲18,800", "▲35,000"],
        ["EBITDA",                "1,820",   "6,480",   "14,400",  "24,000",  "40,000"],
        ["EBITDA率",              "13%",     "18%",     "20%",     "20%",     "20%"],
    ]
...
def fig_headcount_chart():
    ...
    years = ["Y1\n2026", "Y2\n2027", "Y3\n2028", "Y4\n2029", "Y5\n2030"]
    # 役員: Y1-Y2は1名（オーナー）、Y3-Y5は2名
    # 本部・SV・役員合計
    hq      = [2,  4,  7, 10, 12]   # 役員+本部+SV
    # 蕎麦然:社員1名/店、だし然/肉酒場然:社員2名/店
    staff   = [5, 15, 34, 55, 80]   # 店舗社員
    # 蕎麦然:アルバイト2名/店、他:3名/店
    part    = [8, 24, 56, 90, 135]  # アルバイト

    x = np.arange(len(years))
    w = 0.5
    b1 = ax.bar(x, hq,    w, label="本部・SV・役員", color=MPL_NAVY,  alpha=0.9)
...
def slide_hiring_plan(prs):
    ...
    hdrs = ["", "Y1 2026", "Y2 2027", "Y3 2028", "Y4 2029", "Y5 2030"]
    rows = [
        ["店舗数（直営＋FC）",   "3店",   "9店",   "22店",  "35店",  "55店"],
        ["役員（名）",           "1",     "1",     "2",     "2",     "2"],
        ["本部・SV（名）",       "1",     "3",     "5",     "8",     "10"],
        ["店舗社員（名）",       "5",     "15",    "34",    "55",    "80"],
        ["アルバイト（名）",     "8",     "24",    "56",    "90",    "135"],
        ["総人員（名）",         "15",    "43",    "97",    "155",   "227"],
        ["人件費（万円）",       "3,840", "9,200", "16,480","29,000","46,000"],
    ]
    table(slide, hdrs, rows,
          Inches(0.3), Inches(5.1), Inches(12.7), Inches(2.25),
          hdr_color=NAVY, font_size=10)

    txt(slide, "※ 蕎麦然:社員1名＋アルバイト2名/店　だし然・肉酒場然:社員2名＋アルバイト3名/店　役員報酬は別途設計予定",
        Inches(0.4), Inches(7.38), Inches(12.7), Inches(0.28),
...
def slide_11_funding(prs):
    ...
    multi_txt(slide,
        ["■ 投資家へのリターンイメージ",
         ...
         "・Y3以降：FCスケールで利益率が20%に安定。EBITDA1.4億円（月商6,000万）で成長が加速",
