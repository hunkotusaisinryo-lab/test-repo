...
def fig_competitive_map():
    ...
    competitors = [
        ...
        # 業態比較ベンチマーク
        ("やきとり家すみれ", 3.0, 4.5, "#E07B39", 100, False),  # FC展開力高・素材こだわり中程度
    ]
...
def slide_competitor_analysis(prs):
    ...
    rows = [
        ...
        ["やきとり家すみれ", "全国展開",  "FC焼鳥・ファミリー/女性向け",      "¥2,500",  "約90店(FC67)", "★★★☆☆", "★★★★☆", "FCモデルの先行事例・直接競合"],
    ]
    table(slide, hdrs, rows,
          Inches(0.3), Inches(1.5), Inches(12.7), Inches(5.0),
          hdr_color=ACCENT, font_size=9.5)

    # 凡例
    txt(slide, "※ こだわり度・展開可能性は5段階評価（★多い＝高い）　客単価・店舗数は公開情報に基づく推計",
        Inches(0.4), Inches(6.6), Inches(12.0), Inches(0.35),
        size=9, color=TEXT_COLOR, italic=True)

    txt(slide, "→ やきとり家すみれはFC展開力が高いが「素材・発酵コア」の独自性で然グループが差別化できる",
...
def slide_competitor_strategy(prs):
    ...
    insights = [
        ("やきとり家すみれとの差（最重要比較）",
         "• すみれはFC展開力★4.5と高いが「素材・発酵コア」が弱い\n• 然グループは大山どりではなく出汁・麹・発酵という唯一無二の軸を持つ\n• ブランド統一度も低く3ブランド横断シナジーがない"),
        ("ひまり堂・びゃく・三谷との差",
         "• 職人系の高こだわりだが単店止まり・スケールしない\n• 客単価¥8,000超（びゃく）は市場が狭く景気敏感\n• 然グループは「再現可能なこだわり」でFC30店に複製できる"),
        ("ひまり商店・なかよしとの差",
         "• 価格帯は近いが「素材・発酵」の明確なコアがない\n• 然グループは出汁・麹・発酵を共通軸に全ブランドが連動\n• SNS内製でマーケ費ゼロのまま情報優位を維持"),
        ("然グループ固有の強み（競合不在ゾーン）",
         "• 素材こだわり★4.5 × FC展開力★4.2 の両立はすみれも実現できていない\n• 3ブランド構成で景気・トレンドのリスク分散が効く\n• 発酵・出汁コアのR&Dをグループで共有 = 1店あたりコスト最小"),
    ]

    colors_list = [DARK_RED, NAVY, GOLD, ACCENT]
...
def slide_differentiation_deep(prs):
    ...
    fc_points = [
        ("すみれとの決定的な差",
         "やきとり家すみれ（約90店）はFC展開力は高いが\n素材・発酵コアの独自性がない＝差別化軸が弱い\n然グループは「こだわり×スケール」を同時実現"),
        ("ロイヤルティ収入エンジン",
         "売上の3〜4%が直営費用ゼロで入る\n店舗数×ロイヤルティ＝純粋な利益エンジン\n55店舗で月間6,000万円超の安定収入"),
        ("3ブランドのリスク分散",
